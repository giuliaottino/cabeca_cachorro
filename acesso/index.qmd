---
title: "Acesso aos dados - Tsiino Hiiwiida"
...
resources:
  - ../figures/
  - _input_data/
  - ../_input_data/icons/ffb.png
  - ../_input_data/icons/powo.png
---
...
empty_catalog <- function() {
  tibble::tibble(
    ...
    record_id = character(),
    collector = character(),
    collector_display = character(),
    recordNumber = character(),
    ffb_taxon_url = character(),
    powo_taxon_url = character(),
    latitude = numeric(),
    longitude = numeric(),
...
read_dwc_xlsx <- function(path) {
  ...
  dat <- readxl::read_xlsx(path, sheet = sheet, col_types = "text", .name_repair = "unique") |>
...
col_or_na <- function(dat, nm) {
  if (nm %in% names(dat)) dat[[nm]] else rep(NA_character_, nrow(dat))
}

coalesce_nz <- function(...) {
  xs <- list(...)
...
standardize_dwc_cols <- function(dat) {
  if (nrow(dat) == 0) return(dat)

  col <- function(nm) col_or_na(dat, nm)

  fill_col <- function(target, ...) {
    novo <- coalesce_nz(...)
    if (!target %in% names(dat)) return(novo)

    atual <- as.character(dat[[target]])
    atual[is.na(atual) | !nzchar(trimws(atual))] <- NA_character_
    dplyr::coalesce(stringr::str_squish(atual), novo)
  }

  dat$scientificName <- fill_col(
    "scientificName",
    col("scientificName"),
  ...
  dat$taxonRank <- fill_col(
    "taxonRank",
    col("taxonRank"),
    col("verbatimTaxonRank")
  )

  dat$species_key <- fill_col(
    "species_key",
    col("species_key"),
    col("species"),
    col("acceptedScientificName"),
  ...
  dat$collector <- fill_col("collector", col("collector"), col("recordedBy"))
  dat$collector_display <- fill_col("collector_display", col("collector_display"))
  dat$recordNumber <- fill_col("recordNumber", col("recordNumber"))
  dat$ffb_taxon_url <- fill_col("ffb_taxon_url", col("ffb_taxon_url"))
  dat$powo_taxon_url <- fill_col("powo_taxon_url", col("powo_taxon_url"))

  dat$latitude <- fill_col(
    "latitude",
    col("latitude"),
    col("decLat_final"),
    col("decimalLatitude"),
    col("decimalLatitude.new"),
    col("decimalLatitude_plantR")
  )

  dat$longitude <- fill_col(
    "longitude",
    col("longitude"),
    col("decLon_final"),
    col("decimalLongitude"),
    col("decimalLongitude.new"),
    col("decimalLongitude_plantR")
  )

  if (!"genus" %in% names(dat)) dat$genus <- col("genericName")
  if (!"family" %in% names(dat)) dat$family <- col("family")
  if (!"kingdom" %in% names(dat)) dat$kingdom <- col("kingdom")
  if (!"phylum" %in% names(dat)) dat$phylum <- col("phylum")
  if (!"class" %in% names(dat)) dat$class <- col("class")
  if (!"order" %in% names(dat)) dat$order <- col("order")

  rank <- stringr::str_to_upper(clean_text(dat$taxonRank))

  if (!"is_species_level" %in% names(dat) || !any(dat$is_species_level %in% c(TRUE, "TRUE", "true", "1", 1), na.rm = TRUE)) {
    dat$is_species_level <- rank %in% c("SPECIES", "SUBSPECIES", "VARIETY", "FORM")
  }

  if (!"taxon_resolution" %in% names(dat) || !any(nz(dat$taxon_resolution))) {
    dat$taxon_resolution <- dplyr::case_when(
      rank %in% c("SPECIES", "SUBSPECIES", "VARIETY", "FORM") ~ "Até espécie",
      rank == "GENUS" ~ "Até gênero",
      rank == "FAMILY" ~ "Até família",
      TRUE ~ "Indeterminados"
    )
  }

  kingdom <- stringr::str_to_upper(clean_text(col("kingdom")))
  classe <- stringr::str_to_upper(clean_text(col("class")))
  phylum <- stringr::str_to_upper(clean_text(col("phylum")))
  ...
  derived_group <- dplyr::case_when(
    kingdom == "FUNGI" ~ "Fungos",
    kingdom == "PLANTAE" & classe %in% seed_classes ~ "Fanerógamas",
    kingdom == "PLANTAE" & phylum %in% cryptogam_phyla ~ "Criptógamas",
    kingdom == "PLANTAE" & !(classe %in% seed_classes) ~ "Criptógamas",
    TRUE ~ NA_character_
  )
...
read_validated_dwc_input <- function(paths, label, source_label) {
  ...
  if (!is.null(read_error)) {
    return(list(
      ...
      usable_rows = 0L,
      message = paste0(
        label, " encontrado em ", normalizePath(path, winslash = "/", mustWork = FALSE),
        ", mas não pôde ser lido. Erro: ", read_error
      )
    ))
  }

  dat <- standardize_dwc_cols(dat)
  ...
  usable_rows <- dat |>
    filter(group %in% c("Fanerógamas", "Criptógamas", "Fungos")) |>
    nrow()
  ...
  list(
    ...
    message = paste0(
      ...
      ". ", format_int(nrow(dat)), " registros lidos; ",
      format_int(usable_rows), " registros classificados para o painel."
    )
  )
}
...
species_list_df <- function(dat) {
  dat |>
    ...
    summarise(
      records = n(),
      ffb_taxon_url = {
        urls <- ffb_taxon_url[nz(ffb_taxon_url) & grepl("^https?://", ffb_taxon_url)]
        if (length(urls)) urls[[1]] else NA_character_
      },
      powo_taxon_url = {
        urls <- powo_taxon_url[nz(powo_taxon_url) & grepl("^https?://", powo_taxon_url)]
        if (length(urls)) urls[[1]] else NA_character_
      },
      occurrences = list(tibble::tibble(
        collector = collector, collector_display = collector_display,
        recordNumber = recordNumber, source = source
      )),
      .groups = "drop"
    ) |>
...
species_drawer <- function(stats) {
  ...
  dat <- dat |>
    mutate(record_label = dplyr::if_else(records == 1, "1 registro", paste0(format_int(records), " registros")))
  tags$details(
    ...
    tags$div(
      ...
      tags$div(
        ...
        tags$p("A lista mostra apenas nomes completos identificados em nível de espécie, já normalizados no script externo, com família associada. Clique ou passe o cursor sobre o táxon para ver seus registros; os ícones abrem as páginas do táxon na Flora e Funga do Brasil e no Plants of the World Online (POWO).")
      ),
      tags$div(
        class = "species-list-grid",
        lapply(seq_len(nrow(dat)), function(i) {
          occurrences <- dat$occurrences[[i]]
          tags$article(
            class = "species-list-card",
            tags$div(class = "species-family-line", tags$span(dat$family[i])),
            tags$div(
              class = "species-taxon-line",
              tags$button(type = "button", class = "species-taxon-trigger",
                `aria-expanded` = "false", `aria-controls` = "taxon-record-popup",
                tags$em(dat$species_key[i])),
              tags$span(
                class = "taxon-external-links",
                if (nz(dat$ffb_taxon_url[i])) tags$a(
                  class = "taxon-source-link ffb-link", href = dat$ffb_taxon_url[i],
                  target = "_blank", rel = "noopener noreferrer",
                  title = "Flora e Funga do Brasil",
                  `aria-label` = paste(dat$species_key[i], "na Flora e Funga do Brasil"),
                  tags$img(src = "../_input_data/icons/ffb.png", alt = "Flora e Funga do Brasil", width = 28, height = 28)
                ),
                if (nz(dat$powo_taxon_url[i])) tags$a(
                  class = "taxon-source-link powo-link", href = dat$powo_taxon_url[i],
                  target = "_blank", rel = "noopener noreferrer",
                  title = "Plants of the World Online (POWO)",
                  `aria-label` = paste(dat$species_key[i], "no Plants of the World Online"),
                  tags$img(src = "../_input_data/icons/powo.png", alt = "Plants of the World Online (POWO)", width = 28, height = 28)
                )
              )
            ),
            tags$template(
              class = "taxon-record-content",
              tags$strong(dat$species_key[i]),
              tags$p(dat$record_label[i]),
              tags$ul(lapply(seq_len(nrow(occurrences)), function(j) {
                rec <- occurrences[j, ]
                collection_label <- if (nz(rec$collector)) {
                  paste0(rec$collector, " · nº ", if (nz(rec$recordNumber)) rec$recordNumber else "não informado")
                } else if (nz(rec$collector_display)) {
                  rec$collector_display
                } else {
                  paste0("Coletor não informado · nº ", if (nz(rec$recordNumber)) rec$recordNumber else "não informado")
                }
                tags$li(tags$span(collection_label), tags$small(rec$source))
              }))
            )
          )
        })
...
render_dashboard <- function(title, subtitle, group_filter = NULL, compact = FALSE) {
  ...
  kpis <- tags$div(
    ...
    kpi_card(format_int(stats$exp_records), "registros das expedições", paste0(
      if (stats$records > 0) format(round(100 * stats$exp_records / stats$records, 1), decimal.mark = ",", trim = TRUE) else "0",
      "% dos registros deste painel foram coletados nas expedições Tsiino Hiiwiida."
    )),
...
<style>
  ...
  *{ box-sizing: border-box; }
  ...
  .lead{
    ...
    border-radius: 16px;
    background: rgba(215,221,197,0.22);
    padding: 1rem 1.2rem;
  ...
  .species-taxon-line{ display: flex; align-items: center; gap: 0.6rem; }
  .species-taxon-trigger{ border: 0; background: transparent; padding: 0; text-align: left; cursor: pointer; flex: 1; min-width: 0; }
  .species-taxon-trigger:focus-visible, .taxon-source-link:focus-visible{ outline: 2px solid var(--green); outline-offset: 4px; }
  .taxon-external-links{ display: inline-flex; align-items: center; gap: 0.35rem; flex: 0 0 auto; }
  .taxon-source-link{ display: inline-flex; flex: 0 0 auto; border-radius: 6px; }
  .taxon-source-link img{ width: 28px; height: 28px; object-fit: contain; display: block; }
  .powo-link img{ border-radius: 4px; }
  .taxon-record-popup{ position: fixed; z-index: 10001; width: min(390px, calc(100vw - 24px)); max-height: min(420px, calc(100dvh - 24px)); overflow: auto; padding: 1rem; border: 1px solid var(--line); border-radius: 16px; background: var(--paper); color: var(--ink); box-shadow: 0 12px 36px rgba(0,0,0,0.2); font-family: 'Montserrat', Arial, sans-serif; }
  .taxon-record-popup[hidden]{ display: none; }
  .taxon-record-popup p{ margin: 0.5rem 0; font-size: 0.85rem; }
  .taxon-record-popup ul{ margin: 0; padding-left: 1.2rem; }
  .taxon-record-popup li{ padding: 0.45rem 0; font-size: 0.85rem; overflow-wrap: anywhere; }
  .taxon-record-popup small{ display: block; color: var(--muted); margin-top: 0.2rem; }
...
<section id="resultados-indicadores" class="content-card results-section">
  ...
  <p class="reveal" data-reveal>O painel geral e os painéis por grupo mostram o total e a proporção de registros das expedições Tsiino Hiiwiida em relação ao conjunto de dados. O painel geral também mostra o nível de identificação dos registros — até espécie, até gênero, até família ou indeterminados — para acompanhar a qualidade taxonômica da base.</p>
...
render_dashboard(
  ...
  subtitle = "Síntese integrada da flora e funga documentada pelas coleções existentes e pelas expedições atuais. O painel reúne fanerógamas, criptógamas e fungos, evidenciando riqueza, gêneros, famílias, registros, georreferenciamento e a contribuição das expedições do projeto.",
  group_filter = NULL
)
```
...
(function(){
    ...
    if(el.classList && el.classList.contains('chart-card')){
      ...
      if(window.Plotly){
        el.querySelectorAll('.js-plotly-plot').forEach(function(plot){
          setTimeout(function(){ Plotly.Plots.resize(plot); }, 120);
        });
      }
    }
  }
  ...
  function setupTaxonPopups(){
    var popup = document.createElement('div');
    popup.id = 'taxon-record-popup';
    popup.className = 'taxon-record-popup';
    popup.hidden = true;
    popup.setAttribute('role', 'region');
    popup.setAttribute('aria-label', 'Registros do táxon');
    popup.tabIndex = 0;
    ...
    function show(button){
      ...
      if(active !== button){
        close(); active = button;
        popup.replaceChildren(button.closest('.species-list-card').querySelector('template').content.cloneNode(true));
      }
      popup.hidden = false; button.setAttribute('aria-expanded', 'true'); position();
    }
    function scheduleClose(){
      clearTimeout(closeTimer);
      closeTimer = setTimeout(function(){ if(!pinned && !popup.matches(':hover') && !popup.contains(document.activeElement) && active !== document.activeElement) close(); }, 220);
    }
    document.querySelectorAll('.species-taxon-trigger').forEach(function(button){
      button.addEventListener('mouseenter', function(){ if(!pinned) show(button); });
      button.addEventListener('mouseleave', scheduleClose);
      button.addEventListener('focus', function(){ if(!pinned) show(button); });
      button.addEventListener('blur', scheduleClose);
      button.addEventListener('click', function(){
        if(active === button && pinned){ close(); return; }
    ...
    document.addEventListener('scroll', function(event){ if(!popup.contains(event.target)) close(); }, true);
  ...
  function setupPage(){ setupReveal(); setupTaxonPopups(); }
